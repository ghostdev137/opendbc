...
class CarState(CarStateBase, MadsCarState):
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    # Transit PSCM zeros CcStat_D_Actl at standstill even while the main button is on,
    # which flickers cruiseState.available to False and causes MADS to disengage every stop.
    # Fall back to the driver-requested ACC enable flag, which stays stable.
    if self.CP.carFingerprint == CAR.FORD_TRANSIT_MK5:
      ret.cruiseState.available |= cp.vl["Cluster_Info1_FD1"]["AccEnbl_B_RqDrv"] == 1
